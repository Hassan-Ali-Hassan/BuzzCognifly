...
// buzzvm_state TablePut_real1(buzzvm_t vm,buzzobj_t t_table,int n_idx,float f_value) {
   // buzzvm_push(vm, t_table);
   // buzzvm_pushi(vm, n_idx);
...
// int do_voronoi_3(buzzvm_t vm)
// {
//   int argn = buzzvm_lnum(vm);
//   //printf("number of arguments is   %i\n",argn);
//   /*extracting data from the input table (to be passed to the class member function)*/
//   buzzvm_lload(vm, 1); 
//   buzzvm_lload(vm, 2); //this second input should be the index of the robot to identify its position from the beginning
//   buzzvm_type_assert(vm, 2, BUZZTYPE_INT);
//   buzzvm_type_assert(vm, 1, BUZZTYPE_TABLE); 
  

//   int localIndex = buzzvm_stack_at(vm, 2)->i.value;
//   buzzobj_t t = buzzvm_stack_at(vm, 1);
  
//   int table_size = buzzdict_size(t->t.value);
//   // printf("table size \t %i\n",table_size);
//   float a[table_size];
//   float b[table_size];
//   float c[2]; //this will contain the centeroid of the current robot
//   float myPos[2];
//   float max = 0;
//   float min = 0;
...
//     buzzvm_tget(vm);
//     buzzvm_type_assert(vm, 1, BUZZTYPE_FLOAT); 
//     a[i] = buzzvm_stack_at(vm, 1)->f.value; 
//     if(a[i] >= max)
//     {
//       max = a[i];
//     }
...
//   // float basic_dimension = max - min + 50.0;
//   float basic_dimension = 450;
//   // float delta = (abs_min + 10.0);
//   float delta = basic_dimension * 0.5;
//   // printf("Basic dimension is:  %f\n",basic_dimension);
...
//   /*registering the current location of the robot. Notice that 
//   the current location of the robot should be the first two elements
//   it in the input table*/
//   myPos[0] = a[2*(localIndex-1)];
//   myPos[1] = a[2*(localIndex-1)+1];
//   //printf("Read the input table %f and %f \n",a[2],a[3] );

//   //looking for the smallest value
//   /*calculating voronoi*/
//   VoronoiDiagramGenerator vdg = VoronoiDiagramGenerator();
//   vdg.do_voronoi(a,b,myPos,c,table_size/2,basic_dimension); //b is the positions of all voronoi centers, and c is the centroid of the specific local robot
//   if(min < 0)
//   {  
//     for(int i = 0; i < table_size; i++)
//     {
...
//     c[1] -= delta;
//   }
//   /*now we need to push the results back to the buzz script in the form of a table*/
//   buzzvm_pusht(vm); 
//   buzzobj_t tProxTable = buzzvm_stack_at(vm, 1); 

//   for(size_t i = 0; i < 2; ++i) {
//     TablePut_real(vm,tProxTable, i,c[i]);
//   }    
